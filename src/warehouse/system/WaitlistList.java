...
public class WaitlistList implements Serializable{
    ...
    public Iterator getWaitlists(){
     ...
     // Within WaitlistList
    public int fulfill(String productID, int quantity, String manu){
      Iterator iterator = this.getWaitlists();
       while( iterator.hasNext() ) {
        Waitlist waitlist = (Waitlist) iterator.next();
           if (waitlist.needsProduct(productID, manu) && quantity != 0) {
               quantity = waitlist.decrement(productID, quantity, manu);
           }
       }
       return quantity;
   }
